...
router.post('/acp/chat', async (req, res) => {
  ...
  try {
    ...
    // claude-code-acp aborts with "Claude Code cannot be launched inside another
    // Claude Code session" if CLAUDECODE is set on the spawned child. The provider
    // library always merges process.env first, so we have to explicitly override
    // here (empty string is falsy, which the agent's check accepts).
