...
router.post('/acp/chat', async (req, res) => {
  ...
  try {
    ...
    // Strip CLAUDECODE from the child env. claude-code-acp aborts with
    // "Claude Code cannot be launched inside another Claude Code session" if
    // the parent process is itself a Claude Code session. CLAUDE_CODE_ENTRYPOINT
    // is its companion var; remove that too for safety.
    ...
    provider = createACPProvider({
      command,
      args,
      env: { ...parentEnv, ...env, PATH: envPath },
      session: {
        cwd: process.cwd(),
        mcpServers,
      },
