...
import { createSeriesFolder } from '../../services/arcwriteFS';
...
/** Write a markdown file to the series folder via API. */
async function writeSeriesFile(seriesName, filename, content) {
    try {
        await apiPut(
            `/projects/series/${encodeURIComponent(seriesName)}/files/${encodeURIComponent(filename)}`,
            { content }
        );
...
export default function NewSeriesWizard({ isOpen, onClose, colors: c, isDark, initialSeriesName }) {
    // --- State ---
    ...
    const handleStep1Next = useCallback(async () => {
        ...
        try {
            const { getAllSeries } = await import('../../services/database');

            // Check if series already exists — resume rather than duplicate
            const allSeries = await getAllSeries();
            const existing = allSeries.find(
                (s) => s.name.toLowerCase() === seriesName.trim().toLowerCase()
            );
            if (existing) {
                setCreatedSeriesId(existing.id);
                // Load existing metadata to pre-fill step 2
                if (existing.notes) {
                    try {
                        const meta = JSON.parse(existing.notes);
                        if (meta.bookCount) setBookCount(meta.bookCount);
                        if (meta.wordsPerBook) setWordsPerBook(meta.wordsPerBook);
                        if (meta.chaptersPerBook) setChaptersPerBook(meta.chaptersPerBook);
                        if (meta.genre) useAppStore.getState().setGenre?.(meta.genre);
                    } catch { /* not JSON */ }
                }
                // Load it as the active series in the store
                const loadSeries = useSeriesStore.getState().loadSeries;
                if (loadSeries) loadSeries(existing.id);
                console.log('[Wizard] Resuming existing series:', existing.name, existing.id);
                setStep(1);
                return;
            }
            ...
            console.error('[Wizard] Step 1 failed:', err);
    ...
    const handleAcceptMarket = useCallback(async () => {
        ...
        // Extract and write comps.md (Comparable Titles section)
        const compsMatch = marketSurvey.match(/## Comparable Titles([\s\S]*?)(?=\n## |\n$|$)/);
        if (compsMatch) {
            const compsContent = compsMatch[1].trim();
            await writeSeriesFile(seriesName.trim(), 'comps.md',
                `# Comparable Titles: ${seriesName.trim()}\n\n${compsContent}`);
        }
