...
import NewSeriesWizard from './NewSeriesWizard_v2';
...
export default function SeriesManager({ colors: c, isDark, onSeriesChanged }) {
    ...
    const [wizardSeriesName, setWizardSeriesName] = useState('');

    /** Sync all series artifact files to disk (market-survey.md, comps.md, premises.md) */
    const syncSeriesFiles = async () => {
        try {
            const all = useSeriesStore.getState().allSeries;
            console.log('[SeriesManager] Syncing artifact files for', all.length, 'series');
            for (const s of all) {
                if (!s.notes) continue;
                let meta;
                try { meta = typeof s.notes === 'string' ? JSON.parse(s.notes) : s.notes; }
                catch { continue; }

                const writeFile = async (filename, content) => {
                    try {
                        await apiPut(
                            `/projects/series/${encodeURIComponent(s.name)}/files/${encodeURIComponent(filename)}`,
                            { content }
                        );
                        console.log(`[SeriesManager] Wrote ${filename} for "${s.name}"`);
                    } catch (err) {
                        console.warn(`[SeriesManager] Failed to write ${filename}:`, err.message);
                    }
                };

                // market-survey.md
                if (meta.marketResearch) {
                    await writeFile('market-survey.md',
                        `# Market Survey: ${s.name}\n\n${meta.marketResearch}`);

                    // comps.md (extracted from market survey)
                    const compsMatch = meta.marketResearch.match(/## Comparable Titles([\s\S]*?)(?=\n## |\n$|$)/);
                    if (compsMatch) {
                        await writeFile('comps.md',
                            `# Comparable Titles: ${s.name}\n\n${compsMatch[1].trim()}`);
                    }
                }

                // premises.md
                if (meta.premises) {
                    const selected = (meta.selectedPremise !== undefined && meta.selectedPremise !== null)
                        ? `> **Selected premise:** #${meta.selectedPremise + 1}\n\n`
                        : '';
                    await writeFile('premises.md',
                        `# Series Premises: ${s.name}\n\n${selected}${meta.premises}`);
                }
            }
        } catch (err) {
            console.warn('[SeriesManager] artifact sync failed:', err.message);
        }
    };
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (db) {
                    loadAllSeries();
                    // After loading, sync any missing artifact files to disk
                    setTimeout(() => syncSeriesFiles(), 500);
                }
    ...
    const handleWizardClose = async () => {
        ...
        // Sync artifact files after wizard completes
        await syncSeriesFiles();
    };
    ...
    return (
        <div style={{ marginTop: 16 }}>
                ...
                {allSeries.length === 0 ? (
                    ...
                    allSeries.map((s) => (
                        ...
                        >
                            ...
                            {expanded === s.id && (
                                <SeriesDetail seriesId={s.id} colors={c} isDark={isDark} onOpenWizard={(name) => { setWizardSeriesName(name); setShowWizard(true); }} />
                            )}
                        </div>
                    ))
                )}
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <button
                    onClick={() => { setWizardSeriesName(''); setShowWizard(true); }}
                    style={{
            ...
            <NewSeriesWizard
                ...
                colors={c}
                isDark={isDark}
                initialSeriesName={wizardSeriesName}
            />
        </div>
    );
}
...
function SeriesDetail({ seriesId, colors: c, isDark, onOpenWizard }) {
    ...
    useEffect(() => {
        (async () => {
            ...
            setBooks(await db.getBooksBySeriesId(seriesId));
    ...
    return (
        <div style={{ padding: '6px 12px 10px 36px', fontSize: 11 }}>
            ...
            {/* Wizard button */}
            <button
                onClick={() => onOpenWizard?.(seriesName)}
                style={{
                    background: 'linear-gradient(135deg, #7C3AED 0%, #2563EB 100%)',
                    color: '#fff',
                    border: 'none',
                ...
                title="Open the wizard for this series"
            >
                ✨ Wizard
            </button>
